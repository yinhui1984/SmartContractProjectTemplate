...
src = '../contracts/__CONTRACT_NAME__.sol'
contract_src = open(src).read()
print(contract_src)

compiled_sol = solcx.compile_source(contract_src, output_values=['bin', 'abi'])
contract_interface = compiled_sol['<stdin>:__CONTRACT_NAME__']

__CONTRACT_NAME__ = w3.eth.contract(abi=contract_interface['abi'], bytecode=contract_interface['bin'])
w3.eth.default_account = w3.eth.accounts[0]
tx_hash = __CONTRACT_NAME__.constructor().transact()
...
with open("../contracts/__CONTRACT_NAME__.address", "w") as f:
